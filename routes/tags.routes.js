// caminho: plugconversa/routes/tags.routes.js
...
router.post('/move', async (req, res) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM contact_tags WHERE contact_id=$1 AND tag_id=$2;`, [cid, fromId])
    ...
    return res.status(500).json({ error: 'erro ao mover etiqueta', details: String(err?.message || err) })
  } finally {
    client.release()
  }
})
...
router.post('/reorder', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: 'erro ao reordenar colunas', details: String(err?.message || err) })
  } finally {
    client.release()
  }
})
...
// fim do caminho: plugconversa/routes/tags.routes.js
